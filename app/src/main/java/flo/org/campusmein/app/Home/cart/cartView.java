package flo.org.campusmein.app.Home.cart;
...
public class cartView extends AppCompatActivity implements View.OnClickListener,ConnectivityReceiver.ConnectivityReceiverListener {
    ...
    private void showPriceView() {
        ...
        if(Boolean.valueOf(campusExchangeApp.getInstance().getUniversalPerson().getPersonPresent())){
            continueChkOut.setText(getString(R.string.btn_continue));
        }else {
            continueChkOut.setText(R.string.signInBtnCart);
        }
    }
    ...
    private void addToWishList(String product_id) {
        if(RealmController.getInstance().isItemInWishlist(product_id)) {
            Toast.makeText(this, R.string.alreadyInWishlist,Toast.LENGTH_SHORT).show();
        }else {
            RealmController.getInstance().addToWishList(product_id);
...
//                            ActivityCompat.invalidateOptionsMenu();
    ...
    private void setCombopackProperties(Products product, int quantityInt) {
        ...
        try {
            Glide.with(this).load(product.combopack.photoUrl).placeholder(R.drawable.ic_combo_default).thumbnail(0.5f).diskCacheStrategy(DiskCacheStrategy.ALL).into(cart_itemImage);
    ...
    private void setInstrumentProperties(Products product, int quantityInt) {
        ...
        quantity.setText(""+quantityInt);
        try {
            Glide.with(this).load(product.instrument.photoFile).placeholder(R.drawable.ic_instrument_default).thumbnail(0.5f).diskCacheStrategy(DiskCacheStrategy.ALL).into(cart_itemImage);
        }catch (NullPointerException e){
    ...
    private void setBookProperties(Products product, int quantityInt) {
        ...
        quantity.setText(""+quantityInt);
        try {
            Glide.with(this).load(product.book.photofile).placeholder(R.drawable.ic_book_default).thumbnail(0.5f).diskCacheStrategy(DiskCacheStrategy.ALL).into(cart_itemImage);
        }catch (NullPointerException e){
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.continueChkOut:
                ...
                if (NETWORK_STATE){
                    if(Boolean.valueOf(campusExchangeApp.getInstance().getUniversalPerson().getPersonPresent())){
                        Intent priceView=new Intent(this, flo.org.campusmein.app.Home.orderPlacement.priceView.class);
                        startActivity(priceView);
                    }else {
                        Intent signIn = new Intent(this, flo.org.campusmein.app.Login.login.class);
                        signIn.putExtra(SRC_STR_KEY,"cart");
